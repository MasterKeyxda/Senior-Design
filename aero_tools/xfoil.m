...
%     % xtrf =   1.000 (top)        1.000 (bottom)  
% %     P = textscan(fid,' xtrf =   %1.3f (top)        %f (bottom)');%,'Delimiter',' ','MultipleDelimsAsOne',true,'CollectOutput',1,'HeaderLines',3);
% %     pol.xtrf_top = P{1}(1);
% %     pol.xtrf_bot = P{1}(2);
% %     % Mach =   0.000     Re =     1.000 e 6     Ncrit =  12.000
...
%     % data
%     fmt = ['{',repmat('%f32',1,7),'}'];
%     P = textscan(fid,fmt, 'Delimiter',' ','MultipleDelimsAsOne',true,'CollectOutput',1,'HeaderLines',3);
%     fclose(fid);
...
%         disp(tline)
        ii = ii +1;
        if ii > 7
            P = cellfun(@str2num,strsplit(strtrim(tline)));
            pol.alpha(ii-7) = P(1);
            pol.CL(ii-7)  = P(2);
            pol.CD(ii-7)  = P(3);
            pol.CDp(ii-7) = P(4);
            pol.Cm(ii-7)  = P(5);
            pol.Top_xtr(ii-7) = P(6);
            pol.Bot_Xtr(ii-7) = P(7);
        end
        tline = fgetl(fid);
    end
    fclose(fid);
