...
% Fuselage
mXcgi.Fuselage = [Wt.Struc.Fuselage, -XLE_w - 0.25*cRootSub + cglocAC + 0.40*L_F]; % ranges from 0.40 - 0.50 length of fuselage;
% Engine
length_engine = 142/12; % ft length of PW TF33-P7
x_engine = 101.25; % location of inlet of engine from nose
mXcgi.Engine2 = [Wt.Pwr.Engine*(2/3),-XLE_w - 0.25*cRootSub + cglocAC + x_engine + length_engine*.3]; % engine_cg can vary from .3 -.4 of length of engine from inlet 
mXcgi.Engine1 = [Wt.Pwr.Engine*(1/3),-XLE_w - 0.25*cRootSub + cglocAC + x_engine + 1.3*length_engine + length_engine*.3]; % single engine
% Nacelle
...
% Oxygen System
mXcgi.Oxygen = [Wt.Feq.OxygenGD,-XLE_w - 0.25*cRootSub + cglocAC + (53.33+25)/2]; % CG placed in center of habitable cabin 
...
mXcgi.Furn =[Wt.Feq.Furn,(-XLE_w - 0.25*cRootSub + cglocAC +53.33+25)/2]; % CG placed in center of habitable cabin
