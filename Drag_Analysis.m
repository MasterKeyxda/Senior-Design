...
%-----Wing-----%
%Aero and Perf.
RNw = 2.671E7; % RE of wing
Rwf =  1.06; % wing-fuselage interference factor (Figure 5.11 Roskam)
RLS = 1.08; % lifting surface correction factor (Figure 5.12 Roskam) 
Cfw = 0.455 / (((log10(RNw))^2.58)*((1 + 0.144*(M^2))^0.58)); % turbulent flat plate friction coefficient of the wing
Lprime = 1.2; % thickness location parameter; 1.2 for at (t/c)max >= 0.30c
tcWing = 0.0525; % thickness to camber ratio wing (NACA 0012 Airfoil)
CD0.wing = Rwf * RLS * Cfw * (1 + (Lprime * tcWing) + 100* ((tcWing)^4))*(Swet.wing/Sref);  
...
%-----Wing-----%
%Aero and Perf.
RNw = 2.5597E7; % RE of wing
Rwf =  1.07; % wing-fuselage interference factor (Figure 5.11 Roskam)
RLS = 1.08; % lifting surface correction factor (Figure 5.12 Roskam) 
Cfw = 0.455 / (((log10(RNw))^2.58)*((1 + 0.144*(M^2))^0.58)); % turbulent flat plate friction coefficient of the wing
Lprime = 1.2; % thickness location parameter; 1.2 for at (t/c)max >= 0.30c
tcWing = 0.0525; % thickness to camber ratio wing (NACA 0012 Airfoil)
...
figure()
plot(CL.array, CD.total.super,'-c') % Supersonic Cruise Configuration
hold on;
plot(CL.array, CD.total.sub, 'Color', [1 0.2 0]) % Subsonic Cruise Configuration
plot(CL.array, CD.total.TO, 'Color', [0 0.75 0]) % Takeoff Configuration 
plot(CL.array, CD.total.Land,'-b') % Landing Configuration
